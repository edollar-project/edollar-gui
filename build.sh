...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/edollar-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    EDOLLAR_EXEC=edollar.exe
fi
...
cd build
qmake ../edollar-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy monerod to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$EDOLLAR_DIR/bin/$EDOLLAR_EXEC $BIN_PATH
fi
