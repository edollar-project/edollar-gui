                                    ...
                                    const QString &language, bool testnet)
{
    ...
    Edollar::Wallet * w = m_pimpl->createWallet(path.toStdString(), password.toStdString(),
...
Wallet *WalletManager::openWallet(const QString &path, const QString &password, bool testnet)
{
    ...
    Edollar::Wallet * w =  m_pimpl->openWallet(path.toStdString(), password.toStdString(), testnet);
...
Wallet *WalletManager::recoveryWallet(const QString &path, const QString &memo, bool testnet, quint64 restoreHeight)
{
    ...
    Edollar::Wallet * w = m_pimpl->recoveryWallet(path.toStdString(), memo.toStdString(), testnet, restoreHeight);
    m_currentWallet = new Wallet(w);
    return m_currentWallet;
}
                                            ...
                                            quint64 restoreHeight)
{
    ...
    Edollar::Wallet * w = m_pimpl->createWalletFromKeys(path.toStdString(), language.toStdString(), testnet, restoreHeight,
...
{
    return Edollar::Wallet::maximumAllowedAmount();
}
...
{
    return QString::fromStdString(Edollar::Wallet::displayAmount(amount));
}

quint64 WalletManager::amountFromString(const QString &amount) const
{
    return Edollar::Wallet::amountFromString(amount.toStdString());
}

quint64 WalletManager::amountFromDouble(double amount) const
{
    return Edollar::Wallet::amountFromDouble(amount);
}

bool WalletManager::paymentIdValid(const QString &payment_id) const
{
    //CHANGE ME --> remove this 
    return true;
    //return Edollar::Wallet::paymentIdValid(payment_id.toStdString());
}

bool WalletManager::addressValid(const QString &address, bool testnet) const
{
    return Edollar::Wallet::addressValid(address.toStdString(), testnet);
}

bool WalletManager::keyValid(const QString &key, const QString &address, bool isViewKey,  bool testnet) const
{
    std::string error;
    if(!Edollar::Wallet::keyValid(key.toStdString(), address.toStdString(), isViewKey, testnet, error)){
...
{
    return QString::fromStdString(Edollar::Wallet::paymentIdFromAddress(address.toStdString(), testnet));
}
...
{
    Edollar::WalletManagerFactory::setLogLevel(logLevel);
}

void WalletManager::setLogCategories(const QString &categories)
{
    Edollar::WalletManagerFactory::setLogCategories(categories.toStdString());
}
...
QString WalletManager::checkUpdates(const QString &software, const QString &subdir) const
{
  ...
  const std::tuple<bool, std::string, std::string, std::string, std::string> result = Edollar::WalletManager::checkUpdates(software.toStdString(), subdir.toStdString());
  if (!std::get<0>(result))
    return QString("");
...
{
    m_pimpl =  Edollar::WalletManagerFactory::getWalletManager();
}
