...
#include "wallet/wallet2_api.h" // we need to have an access to the Edollar::Wallet::Status enum here;
...
public:
    ...
    enum Status {
        Status_Ok       = Edollar::Wallet::Status_Ok,
        Status_Error    = Edollar::Wallet::Status_Error,
        Status_Critical = Edollar::Wallet::Status_Critical
    };

    Q_ENUM(Status)

    enum ConnectionStatus {
        ConnectionStatus_Connected       = Edollar::Wallet::ConnectionStatus_Connected,
        ConnectionStatus_Disconnected    = Edollar::Wallet::ConnectionStatus_Disconnected,
        ConnectionStatus_WrongVersion    = Edollar::Wallet::ConnectionStatus_WrongVersion
    };
    ...
    //! creates transaction to multiple address
    Q_INVOKABLE PendingTransaction * createTransactionMany(const QVariantMap &dest,
    ...
    //! creates async transaction
    Q_INVOKABLE void createTransactionManyAsync(const QVariantMap &dest,
                                            quint32 mixin_count, PendingTransaction::Priority priority);
...
private:
    ...
    Wallet(Edollar::Wallet *w, QObject * parent = 0);
    ~Wallet();
private:
    friend class WalletManager;
    friend class WalletListenerImpl;
    //! libwallet's
    Edollar::Wallet * m_walletImpl;
