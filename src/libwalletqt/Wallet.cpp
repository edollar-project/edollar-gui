...
class WalletListenerImpl : public  Edollar::WalletListener
{
public:
...
{
    QFuture<Edollar::Wallet::ConnectionStatus> future = QtConcurrent::run(m_walletImpl, &Edollar::Wallet::connected);
    QFutureWatcher<Edollar::Wallet::ConnectionStatus> *connectionWatcher = new QFutureWatcher<Edollar::Wallet::ConnectionStatus>();

    connect(connectionWatcher, &QFutureWatcher<Edollar::Wallet::ConnectionStatus>::finished, [=]() {
        QFuture<Edollar::Wallet::ConnectionStatus> future = connectionWatcher->future();
                                              ...
                                              PendingTransaction::Priority priority)
{
    ...
    Edollar::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), amount, mixin_count,
                static_cast<Edollar::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
    PendingTransaction * result = new PendingTransaction(ptImpl,0);
    return result;
}

PendingTransaction *Wallet::createTransactionMany(const QVariantMap &dest,
                                                       quint32 mixin_count, PendingTransaction::Priority priority)
{
    std::set<uint32_t> subaddr_indices;
    std::map<std::string, uint64_t> destination;
    QVariantMap::const_iterator it = dest.constBegin();
    while (it != dest.constEnd()) {
        destination.insert(std::make_pair(it.key().toStdString(), (uint64_t)it.value().toULongLong()));
        ++it;
    }
    Edollar::PendingTransaction * ptImpl = m_walletImpl->createTransactionMany(destination, mixin_count,
                                static_cast<Edollar::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
    PendingTransaction * result = new PendingTransaction(ptImpl, 0);
    return result;
}
...
void Wallet::createTransactionManyAsync(const QVariantMap &dest,
                                quint32 mixin_count, PendingTransaction::Priority priority)
{
    QFuture<PendingTransaction*> future = QtConcurrent::run(this, &Wallet::createTransactionMany,
                                  dest, mixin_count, priority);
    QFutureWatcher<PendingTransaction*> * watcher = new QFutureWatcher<PendingTransaction*>();
    
    QString addr = "";
    QString fake = "";
    connect(watcher, &QFutureWatcher<PendingTransaction*>::finished,
            this, [this, watcher,addr, fake, mixin_count]() {
        QFuture<PendingTransaction*> future = watcher->future();
        watcher->deleteLater();
        emit transactionCreated(future.result(),fake, fake, mixin_count);
    });
    watcher->setFuture(future);
}
                                                 ...
                                                 quint32 mixin_count, PendingTransaction::Priority priority)
{
    std::set<uint32_t> subaddr_indices;
    Edollar::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), Edollar::optional<uint64_t>(), mixin_count,
                static_cast<Edollar::PendingTransaction::Priority>(priority), currentSubaddressAccount(), subaddr_indices);
...
{
    Edollar::PendingTransaction * ptImpl = m_walletImpl->createSweepUnmixableTransaction();
    PendingTransaction * result = new PendingTransaction(ptImpl, this);
    return result;
}
...
UnsignedTransaction * Wallet::loadTxFile(const QString &fileName)
{
    ...
    Edollar::UnsignedTransaction * ptImpl = m_walletImpl->loadUnsignedTx(fileName.toStdString());
...
{
    //CHANGE ME --> remove this function 
    return QString("");
    //return QString::fromStdString(Edollar::Wallet::genPaymentId());
}
...
{
    //CHANGE ME --> implement this function in wallet c++
    return QString("");
    // uint64_t received;
    // bool in_pool;
...
bool Wallet::parse_uri(const QString &uri, QString &address, QString &payment_id, uint64_t &amount, QString &tx_description, QString &recipient_name, QVector<QString> &unknown_parameters, QString &error)
{
   ...
   bool res= m_walletImpl->parse_uri(uri.toStdString(), s_address, amount, s_tx_description, s_recipient_name, s_unknown_parameters, s_error);
   if(res)
   {
...
{
    return QString::fromStdString(m_walletImpl->getDefaultDataDir()) + "/edollar.log";
}

QString Wallet::getWalletLogPath() const
{
    return QCoreApplication::applicationDirPath() + "/edollar-wallet-gui.log";
}

Wallet::Wallet(Edollar::Wallet *w, QObject *parent)
    : QObject(parent)
    , m_walletImpl(w)
    , m_history(nullptr)
...
Wallet::~Wallet()
{
    ...
    delete m_subaddress;
    m_subaddress = NULL;
    //Edollar::WalletManagerFactory::getWalletManager()->closeWallet(m_walletImpl);
