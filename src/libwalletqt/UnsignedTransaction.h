...
public:
    enum Status {
        Status_Ok       = Edollar::UnsignedTransaction::Status_Ok,
        Status_Error    = Edollar::UnsignedTransaction::Status_Error,
        Status_Critical    = Edollar::UnsignedTransaction::Status_Critical
    };
    Q_ENUM(Status)
...
private:
    explicit UnsignedTransaction(Edollar::UnsignedTransaction * pt, Edollar::Wallet *walletImpl, QObject *parent = 0);
    ~UnsignedTransaction();
private:
    friend class Wallet;
    Edollar::UnsignedTransaction * m_pimpl;
    QString m_fileName;
