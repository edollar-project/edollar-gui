...
{
    // prepare command line arguments and pass to edollard
    ...
    qDebug() << "starting edollard " + m_edollard;
    ...
    // Start edollard
    bool started = m_daemon->startDetached(m_edollard, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(testnet)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM edollard.exe");
#else
                QProcess::execute("pkill edollard");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./edollard status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd,bool testnet, QString &message) const
{
    ...
    p.start(m_edollard, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to edollard
#ifdef Q_OS_WIN
    m_edollard = QApplication::applicationDirPath() + "/edollard.exe";
#elif defined(Q_OS_UNIX)
    m_edollard = QApplication::applicationDirPath() + "/edollard";
#endif

    if (m_edollard.length() == 0) {
