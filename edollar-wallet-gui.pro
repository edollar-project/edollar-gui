...
TEMPLATE = app
TARGET = edollar-wallet-gui

QT += qml quick core widgets
CONFIG += c++11

WALLET_ROOT=$$PWD/edollar
...
linux {
   ...
   #     }
    ...
    # currently edollar has an issue with "static" build and linunwind-dev,
...
TRANSLATIONS =  \ # English is default language, no explicit translation file
                $$PWD/translations/edollar-core.ts \ # translation source (copy this file when creating a new translation)
...
android{
    deploy.commands += make install INSTALL_ROOT=$$DESTDIR && androiddeployqt --input android-libedollar-wallet-gui.so-deployment-settings.json --output $$DESTDIR --deployment bundled --android-platform android-21 --jdk /usr/lib/jvm/java-8-openjdk-amd64 -qmldir=$$PWD
}
...
DISTFILES += \
    notes.txt \
    edollar/src/wallet/CMakeLists.txt \
    ...
    components/SeedAndKeys.qml \
    wizard/WizardCreateViewOnlyWalletUI.qml \
    components/AddressBookWindow.qml \
    components/AddressBookWindow.qml \
    components/TransferManyTable.qml \
    components/SwitchButton.qml \
    components/ExclusiveNodeTable.qml \
    components/ExclusiveNodeWindow.qml


# windows application icon
RC_FILE = edollar-core.rc
